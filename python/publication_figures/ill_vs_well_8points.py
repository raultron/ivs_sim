...
D8pIll = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
#Results for 5 points well conditioned (after gradient)
D8pWell = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
#Results for 5 points in a square
D8pSquare = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
...
for cam in cams:
  ...
  D8pIll.Camera.append(cam)  
  D8pIll.ObjectPoints.append(pl.get_points())
  D8pIll.calculate_metrics()
  ...
  D8pWell.Camera.append(cam)  
  D8pWell.ObjectPoints.append(new_objectPoints)
  D8pWell.calculate_metrics()
  
  
  
print np.median(np.array(D8pIll.Homo_CV_mean)/np.array(D8pWell.Homo_CV_mean))
print np.median(np.array(D8pIll.pnp_tvec_error_mean)/np.array(D8pWell.pnp_tvec_error_mean))
print np.median(np.array(D8pIll.CondNumber)/np.array(D8pWell.CondNumber))
...
pickle.dump([D8pIll,D8pWell], open( "icra_sim_illvsWell_8points.p", "wb" ) )
