...
D5pIll = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
#Results for 5 points well conditioned (after gradient)
D5pWell = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
#Results for 5 points in a square
D5pSquare = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
...
for cam in cams:
  ...
  D5pIll.Camera.append(cam)  
  D5pIll.ObjectPoints.append(pl.get_points())
  D5pIll.calculate_metrics()
...
print np.median(np.array(D5pIll.pnp_tvec_error_mean)/np.array(D5pWell.pnp_tvec_error_mean))
