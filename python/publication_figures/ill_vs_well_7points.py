...
D7pIll = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
#Results for 5 points well conditioned (after gradient)
D7pWell = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
#Results for 5 points in a square
D7pSquare = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
...
for cam in cams:
  ...
  D7pIll.Camera.append(cam)  
  D7pIll.ObjectPoints.append(pl.get_points())
  D7pIll.calculate_metrics()
  ...
  D7pWell.Camera.append(cam)  
  D7pWell.ObjectPoints.append(new_objectPoints)
  D7pWell.calculate_metrics()
  
  
  
print np.median(np.array(D7pIll.Homo_CV_mean)/np.array(D7pWell.Homo_CV_mean))
print np.median(np.array(D7pIll.pnp_tvec_error_mean)/np.array(D7pWell.pnp_tvec_error_mean))
print np.median(np.array(D7pIll.CondNumber)/np.array(D7pWell.CondNumber))
...
pickle.dump([D7pIll,D7pWell], open( "icra_sim_illvsWell_7points.p", "wb" ) )
