...
D6pIll = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
#Results for 5 points well conditioned (after gradient)
D6pWell = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
#Results for 5 points in a square
D6pSquare = DataSimOut(n,pl,validation_plane, ImageNoise = 4, ValidationIters = 1000)
...
for cam in cams:
  ...
  D6pIll.Camera.append(cam)  
  D6pIll.ObjectPoints.append(pl.get_points())
  D6pIll.calculate_metrics()
  ...
  D6pWell.Camera.append(cam)  
  D6pWell.ObjectPoints.append(new_objectPoints)
  D6pWell.calculate_metrics()
  
  
  
print np.median(np.array(D6pIll.Homo_CV_mean)/np.array(D6pWell.Homo_CV_mean))
print np.median(np.array(D6pIll.pnp_tvec_error_mean)/np.array(D6pWell.pnp_tvec_error_mean))
print np.median(np.array(D6pIll.CondNumber)/np.array(D6pWell.CondNumber))
...
pickle.dump([D6pIll,D6pWell], open( "icra_sim_illvsWell_6points.p", "wb" ) )
